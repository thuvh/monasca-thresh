...
if [ "${NO_STORM_CLUSTER}" = "true" ]; then
  ...
  # shellcheck disable=SC2086
  java $JAVAOPTS -classpath "/monasca-thresh.jar:/apache-storm-1.1.1/lib/*" monasca.thresh.ThresholdingEngine /etc/monasca/thresh-config.yml thresh-cluster local
  exit $?
fi
...
mkdir /root/.m2
templer -v -f /settings.xml.j2 /root/.m2/settings.xml
