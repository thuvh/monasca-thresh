...
public class StatsdMetricConsumer implements IMetricsConsumer {
	...
	String defaultDimensions = monascaStatsdDimPrefix + "{\"hostname\":\"localhost\",\"service\":\"monitoring\",\"component\":\"storm\"}";
	...
	//
	// This is the Monasca specific string that adds the dimension element to StatsD
	...
	//
	// Note: You only know that "|#" is a delimeter by looking at the Monasca Python Agent code
	//       since the Monasca StatsD server is a derivative of what the general purpose StatsD
	//       implements and it is executed in the Monasca Agent which was forked from DataDog.
	//       It extends UDP data by postfixing a json struct describing the dimensions.
	//
	...
	public void prepare(Map stormConf, Object registrationArgument,
		...
		try {
			...
			udpclient = new NonBlockingUdpSender(statsdHost, statsdPort, Charset.defaultCharset(),
					handler);
		}
		catch (IOException e) {
			logger.error("{}", e);
	...
	public void report(String s, int number, String dimensions) {
		String statsdMessage = s + ":" + String.valueOf(number) + "|c" + statsdDimensions;
