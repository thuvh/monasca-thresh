...
public class AlarmCreationBolt extends BaseRichBolt {
  ...
  private List<Alarm> finishesAlarm(
    ...
    if (waitingAlarms.isEmpty()) {
      ...
      if (alarmIsComplete(newAlarm)) {
        ...
        this.saveAlarmWithMetric(metric, newAlarm);
        result.add(newAlarm);
      } else {
      ...
      for (final Alarm waiting : waitingAlarms) {
        ...
        if (alarmIsComplete(waiting)) {
          ...
          this.saveAlarmWithMetric(metric, waiting);
          result.add(waiting);
        }
      }
    }
  ...
  protected void checkSporadic(final Metric metric, final Alarm alarm) {
    // run through sub alarm to mark sporadic where needed prior to saving
    final Collection<SubAlarm> subAlarms = alarm.getSubAlarms();
    final Set<MetricDefinitionAndTenantId> alarmedMetrics = alarm.getAlarmedMetrics();

    logger.trace("Analyzing {} sub alarms for alarm {}", subAlarms.size(), alarm.getId());
    ...
    for (final SubAlarm subAlarm : subAlarms) {
      final Optional<MetricDefinitionAndTenantId> match = FluentIterable
          ...
          .firstMatch(new Predicate<MetricDefinitionAndTenantId>() {
            ...
            public boolean apply(@Nullable final MetricDefinitionAndTenantId input) {
              if (input != null) {
                final String defName = input.metricDefinition.name;
                final String metricName = metric.name;

                return defName.equalsIgnoreCase(metricName)
                    && metricFitsInAlarm(
                    alarm,
                    lookUpAlarmDefinition(alarm.getAlarmDefinitionId()),
                    input
                );
              }
              return false;
      ...
      if (match.isPresent()) {
        ...
        subAlarm.setSporadicMetric(metric.isSporadic());
      }
  ...
  private void saveAlarmWithMetric(final Metric metric, final Alarm alarm) {
    this.checkSporadic(metric, alarm);
  ...
  protected AlarmDefinition lookUpAlarmDefinition(String alarmDefinitionId) {
