        ...
        if (EventProcessingBolt.METRIC_ALARM_EVENT_STREAM_ID.equals(tuple.getSourceStreamId())) {
          ...
          if (EventProcessingBolt.CREATED.equals(eventType)) {
            synchronized (SENTINAL) {
              final AlarmDefinitionCreatedEvent event = (AlarmDefinitionCreatedEvent) tuple.getValue(1);
              final AlarmDefinition alarmDefinition = new AlarmDefinition(event.alarmDefinitionId, event.tenantId, event.alarmName, event.alarmDescription, new AlarmExpression(event.alarmExpression), true);
