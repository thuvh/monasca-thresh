...
public class Alarm extends AbstractEntity {
  ...
  public Alarm(AlarmDefinition alarmDefinition) {
    ...
    this.state = this.isDeterministic() ? AlarmState.UNDETERMINED : AlarmState.OK;
   ...
   *
   * <b>Note:</b> All sub alarms need to be non-deterministic to keep alarm out of
   * {@link AlarmState#UNDETERMINED} state.
   * Otherwise alarm acts as if all sub alarms are deterministic.
   *
  ...
  public boolean evaluate(AlarmExpression expression) {
    ...
    if (uninitialized) {
      ...
      if (!this.isDeterministic()) {
        ...
        final String detailMsg = String.format(
            "All sub alarms were non deterministic for alarm %s",
            this.getId()
        );

        this.state = AlarmState.OK;
        this.stateChangeReason = String.format("%s\n%s", rawMsg, detailMsg);
   ...
   *
   * All sub alarms need to be non-deterministic for entire
   * alarm to be such. Otherwise alarm is deterministic
   * (i.e. if at least one sub alarm is deterministic).
   *
   * @return true/false
   *
   * @see SubAlarm#isDeterministic()
