...
public class AlarmCreationBoltTest {
  ...
  public void testCheckSporadicFitsInSubAlarmExpr() {
    ...
    final MetricDefinitionAndTenantId mdti = new MetricDefinitionAndTenantId(metricDefinition, "1");
    final Metric metric = new Metric(metricDefinition, 1, 100, new HashMap<String, String>());
    metric.setPeriod(-1);

    final AlarmDefinition def = this.createAlarmDefinition("max(cpu{hostname=eleanore}) > 90");
    final Alarm alarm = new Alarm(def, AlarmState.UNDETERMINED);
    alarm.setAlarmedMetrics(Sets.newHashSet(mdti));

    bolt.checkSporadic(metric, alarm);

    assertFalse(alarm.getSporadicSubAlarms().isEmpty());
  }
  ...
  public void testCheckSporadicDoesNotFitInSubAlarmExpr() {
    ...
    assertTrue(alarm.getSporadicSubAlarms().isEmpty());
  }
