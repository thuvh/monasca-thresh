...
public class MetricAggregationBoltTest {
  ...
  private Tuple createMetricTuple(final MetricDefinition metricDef, final Metric metric) {
    ...
    return Testing.testTuple(Arrays.asList(new TenantIdAndMetricName(TENANT_ID, metric.name),
        new MetricDefinitionAndTenantId(metricDef, TENANT_ID), metric), tupleParam);
  }
