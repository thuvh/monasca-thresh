...
public class AlarmDAOImplTest {
  ...
  protected void beforeMethod() {
    ...
    handle.execute("truncate table sub_alarms");
  ...
  public void shouldFindForAlarmDefinitionId() {
    // Create two alarms for first AlarmDefinition
    // Create another AlarmDefinition
    // Create one alarm for second AlarmDefinition
    // Get all alarms for first one, should return two
    // Get all alarms for second one, should return one
    ...
    final AlarmDefinition secondAlarmDef =
        new AlarmDefinition(getNextId(), TENANT_ID, "Second", null, new AlarmExpression(
            "avg(cpu{disk=vda, instance_id=123}) > 10"), true);
