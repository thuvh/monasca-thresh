...
public class AlarmSqlImplTest {
  ...
  protected void prepareData() {
    ...
    this.sporadicAlarmDef = new AlarmDefinition(
        ...
        new AlarmExpression("count(log.error{path=/var/log/test},sporadic=true,20) > 5"),
        "HIGH",
        SPORADIC_ALARM_ENABLED,
        Lists.<String>newArrayList()
    );

    HibernateUtil.insertAlarmDefinition(this.sessionFactory.openSession(), this.alarmDef);
    HibernateUtil.insertAlarmDefinition(this.sessionFactory.openSession(), this.sporadicAlarmDef);
  ...
  @Test(groups = "orm")
  public void shouldFindSporadicAlarmDefinition() {
    ...
    this.verifyAlarmList(dao.findForAlarmDefinitionId(this.sporadicAlarmDef.getId()));

    // create two alarms
    final Alarm firstAlarm = new Alarm(this.sporadicAlarmDef, AlarmState.OK);
    final Alarm secondAlarm = new Alarm(this.sporadicAlarmDef, AlarmState.UNDETERMINED);
    ...
    // and check
    final List<Alarm> found = dao.findForAlarmDefinitionId(this.sporadicAlarmDef.getId());
    ...
    // ensure sporadic has been saved
    for (final Alarm alarm : found) {
      assertEquals(1, alarm.getSporadicSubAlarms().size());
    }
  }
}
