...
public class AlarmSqlImplTest {
  ...
  protected void prepareData() {
    ...
    this.nonDeterministicAlarmDef = new AlarmDefinition(
        ...
        new AlarmExpression("count(log.error{path=/var/log/test},deterministic=false,20) > 5"),
        "HIGH",
  ...
  public void shouldFindNonDeterministicAlarmDefinition() {
    ...
    this.verifyAlarmList(dao.findForAlarmDefinitionId(this.nonDeterministicAlarmDef.getId()));

    // create two alarms
    final Alarm firstAlarm = new Alarm(this.nonDeterministicAlarmDef, AlarmState.OK);
    final Alarm secondAlarm = new Alarm(this.nonDeterministicAlarmDef, AlarmState.UNDETERMINED);
    ...
    // and check
    final List<Alarm> found = dao.findForAlarmDefinitionId(this.nonDeterministicAlarmDef.getId());
    ...
    for (final Alarm alarm : found) {
      assertEquals(1, alarm.getNonDeterministicSubAlarms().size());
    }
  }
}
