...
public class HibernateUtil {
  ...
  public static void insertAlarmDefinition(Session session, AlarmDefinition ad) {
    AlarmDefinitionDb alarmDefininitionDb = null;
    try {
      session.beginTransaction();

      alarmDefininitionDb = new AlarmDefinitionDb();
      alarmDefininitionDb.setId(ad.getId());
      alarmDefininitionDb.setTenant_id(ad.getTenantId());
      alarmDefininitionDb.setName(ad.getName());
      alarmDefininitionDb.setDescription(ad.getDescription());
      alarmDefininitionDb.setSeverity(AlarmSeverity.LOW);
      alarmDefininitionDb.setExpression(ad.getAlarmExpression().getExpression());
      alarmDefininitionDb.setMatch_by(ad.getMatchBy().isEmpty() ? null : COMMA_JOINER.join(ad.getMatchBy()));
      alarmDefininitionDb.setActions_enabled(ad.isActionsEnabled() ? 1 : 0);
      alarmDefininitionDb.setCreated_at(new DateTime());
      alarmDefininitionDb.setUpdated_at(new DateTime());
      alarmDefininitionDb.setDeleted_at(null);

      session.save(alarmDefininitionDb);
      ...
      for (final SubExpression subExpression : ad.getSubExpressions()) {
        ...
        SubAlarmDefinitionDb subAlarmDef =
            new SubAlarmDefinitionDb(subExpression.getId(), ad.getId(), alarmSubExpr.getFunction().name(), alarmSubExpr.getMetricDefinition().name,
                alarmSubExpr.getOperator().name(), alarmSubExpr.getThreshold(), alarmSubExpr.getPeriod(), alarmSubExpr.getPeriods(), new DateTime(),
                new DateTime());
        session.save(subAlarmDef);

        for (final Map.Entry<String, String> entry : alarmSubExpr.getMetricDefinition().dimensions.entrySet()) {

          SubAlarmDefinitionDimensionDb dimension = new SubAlarmDefinitionDimensionDb(subExpression.getId(), entry.getKey(), entry.getValue());
