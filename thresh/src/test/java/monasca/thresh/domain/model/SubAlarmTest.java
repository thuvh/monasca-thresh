...
public class SubAlarmTest {
  ...
  public void shouldBeNonDeterministicIfKeywordFoundAndSetToFalse() {
    final SubAlarm subAlarm = this.getSubAlarm("count(log.error{},deterministic=false,1)>1");
    assertFalse(subAlarm.isDeterministic());
    assertEquals(subAlarm.getState(), AlarmState.OK);
  }

  public void shouldBeDeterministicByDefault() {
  ...
  public void shouldBeDeterministicIfExpressionMissing() {
    final SubAlarm subAlarm = this.getSubAlarm("min(hpcs.compute.cpu{id=5}, 60) > 3 times 3");
