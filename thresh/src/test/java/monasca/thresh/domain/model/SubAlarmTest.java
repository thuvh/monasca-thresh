...
public class SubAlarmTest {
  ...
  public void shouldBeSporadicIfExpressionSetAndSporadic() {
    final SubAlarm subAlarm = this.getSubAlarm("count(log.error{},sporadic=true,1)>1");
    assertTrue(subAlarm.isSporadicMetric());
    assertEquals(subAlarm.getState(), AlarmState.OK);
  }

  public void shouldNotBeSporadicIfExpressionSetAndNotSporadic() {
    final SubAlarm subAlarm = this.getSubAlarm("min(hpcs.compute.cpu{id=5}, 60) > 3 times 3");
    assertFalse(subAlarm.isSporadicMetric());
    assertEquals(subAlarm.getState(), AlarmState.UNDETERMINED);
  }

  public void shouldNotBeSporadicIfExpressionMissing() {
    final SubAlarm subAlarm = this.getSubAlarm("min(hpcs.compute.cpu{id=5}, 60) > 3 times 3");
    subAlarm.setExpression(null);
    assertFalse(subAlarm.isSporadicMetric());
    assertEquals(subAlarm.getState(), AlarmState.UNDETERMINED);
  }
