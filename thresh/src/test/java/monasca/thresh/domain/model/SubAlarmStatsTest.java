...
public class SubAlarmStatsTest {
  ...
  public void shouldNotAllowSubAlarmTransitionFromOkToUndetermined_Nondeterministic(){
    final String expression = "sum(log.error{path=/var/log/test.log},deterministic=false,240) >= 100";
    ...
    // initially in OK because nondeterministic
    ...
    for (int i = 0; i < 1080; i++) {
      ...
      if ((t1 % 60) == 2) {
        stats.evaluateAndSlideWindow(t1, 1);
        if (i <= subExpr.getAlarmSubExpression().getPeriod()) {
          // Haven't waited long enough to evaluate, but this is sporadic, so it should be in ok
          assertEquals(stats.getSubAlarm().getState(), AlarmState.OK);
        } else {
          assertEquals(stats.getSubAlarm().getState(), AlarmState.ALARM);
        }
